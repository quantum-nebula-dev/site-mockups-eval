...
- **2026-07-16 — modèle (`#viz`) VALIDÉ** : mosaïque existante gardée + **photos duotone** dans les tuiles (photo N&B teintée dans la couleur de chaque tuile, voiles haut/bas pour la lisibilité). MAIS redondance actée : #domaines et la mosaïque listent les 5 mêmes domaines → **décision user : la mosaïque photos devient LA section domaines ; le design rail sticky + liste indexée est RÉAFFECTÉ à `#approche` (4 principes)**. Titres/leads à ajuster à la passe copy. À exécuter en arrivant à la section approche.
