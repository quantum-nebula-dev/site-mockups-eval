...
3. **Plancher déterministe** (aucun modèle : sonde overflow 390, greps H1/lorem/couleurs hors tokens, diff de scope) ;
4. **Juge GPT-5.5 UNIQUE** (R17 : plus AUCUN critique même-famille — angles morts partagés avec le designer) : rubric + comparatif vs ancre étalon + double verdict (judge.py, effort high), diagnostic JAMAIS prescription, à froid (jamais les verdicts antérieurs) ; caps : objectif 2 tours, créatif 1 tour puis retour direction user ;
...
**RÈGLE DE CRÉATION (clarifiée par l'user 2026-07-03 soir — remplace « élever, jamais remplacer ») :**
- Ce que l'user AIME = **VERROUILLÉ**, intouchable (baselines).
- Ce qui est REJETÉ ou NOUVEAU = **conception FROM SCRATCH** : le designer ne voit JAMAIS les anciennes versions de la section (fixation de design — l'exposition à une solution existante fait recycler ses traits, Jansson & Smith 1991 + revues) ; l'album de goût sert à CALIBRER (niveau, registre, rejets), JAMAIS à recycler des formes. Incident du jour : la treemap « élevée » en marqueterie = fixation, rejetée (« Paint à nouveau »).
- Le from-scratch se fait en **3 CONCEPTS PARALLÈLES divergents** (fichiers isolés `_pipeline/concepts/`, axes différents, corps de section retiré des copies) → **l'user choisit ou fusionne** (parallel prototyping, chiffré : choisir le meilleur = +56 %, fusionner = +70 %, itérer ensuite = +152 %, NN/g ; itérer sur un seul concept = fixation) → polish du choisi → juge → gate user.
- « Élever une baseline aimée » ne s'applique QUE si l'user demande explicitement de retoucher une section qu'il aime.
Ordre restant : **citation (aimée — ne pas toucher) → footer (à créer) → nav (directive user : structure C + police ancienne, gate) → cohérence → passe copy (user) → ship**. (Intro : AIMÉE, retirée du backlog.)
**⏳ PÉRIODE D'ESSAI (user, 2026-07-03)** : le nouveau régime est jugé AU RÉSULTAT sur les prochaines sections (treemap #viz, repères, approche). L'user se réserve le droit de REVENIR à l'ancien /design-section manuel (« pas sûr de pas revert ») — versions antérieures des skills récupérables via git. Chaque livraison doit donc viser SON niveau, pas un score.
**⚠️ Verdicts user 2026-07-03 sur TOUTE la page (à jour, remplace tout ce qui précède)** :
- **AIMÉES (ne pas toucher / élever seulement sur demande)** : hero (figé) ; **intro `#perimetre`** (« la première section en dessous du hero qu'on a faite avant ce site » — RETIRÉE du backlog de refonte) ; **domaines** (restauré + verrouillé, d11c49d) ; **citation `.quote-block`** (« celle avec la quote sur le bandeau photo ») ; **treemap à carrés** (variante A `#viz` : « les cubes, j'aimais bien » → c'est la BASELINE de la refonte du modèle, screenshot au gold).
- **REJETÉES (« dégueulasses… vraiment cheap »)** : `#viz` version schéma fan-in/fan-out (« PowerPoint sous Paint ; trop d'infos, dépasse l'écran ; asset doit être bien plus qualitatif/luxe ») ; `#reperes` ; `#approche`.
- **PASSABLES, retravail bienvenu non prioritaire** : `#contact`, footer (« ça allait mais si on peut les retravailler c'est cool aussi »).
**#viz : la marqueterie (élévation treemap) REJETÉE user (« franchement dégueulasse encore, on dirait du Paint à nouveau ») après 2 BLOCK GPT-5.5 — le juge cross-famille a vu juste DEUX FOIS avant l'user. Treemap d'origine PARQUÉE en attendant (commit 93bb850). 3 CONCEPTS FROM SCRATCH EN PARALLÈLE lancés : designer-viz-fsA (typographique) / fsB (asset abstrait, treemap+diagramme interdits) / fsC (matière/image), fichiers `_pipeline/concepts/viz-{A,B,C}.html` (corps de section retiré des copies, anti-fixation) → gate user à 3 propositions (choix/fusion). Ensuite : #reperes → #approche (FROM SCRATCH aussi, même protocole) → (optionnel : #contact, footer).**
