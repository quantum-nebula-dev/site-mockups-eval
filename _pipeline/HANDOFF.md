...
**Un existant aimé s'ÉLÈVE, ne se remplace jamais** (« from scratch, n'en garde rien » = anti-pattern interdit). Ordre restant : **citation (aimée — ne pas toucher) → footer (à créer) → nav (directive user : structure C + police ancienne, gate) → cohérence → passe copy (user) → ship**. (Intro : AIMÉE, retirée du backlog.)
...
**Refontes sous le NOUVEAU régime, EN FILE (une à la fois — même fichier) : #viz EN COURS (designer-viz-luxe, mandat = ÉLEVER la treemap A au niveau luxe, contenue ~1 écran) → puis #reperes → #approche → (optionnel : #contact, footer).** Chaque refonte passe les filtres puis gate user en page live.
