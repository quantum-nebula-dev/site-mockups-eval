...
**⚠️ Sections repères / approche / contact = produites sous l'ANCIEN régime, validées machine uniquement (0.82-0.88) → statut CANDIDATES : à montrer à l'user en live pour verdict avant tout verrouillage.** Domaines = restauré version originale + VERROUILLÉ (commit d11c49d). **#viz = REJETÉ user 2026-07-03** (« PowerPoint fait sous Paint, cheap ; trop d'infos, dépasse l'écran ; l'asset doit être bien plus qualitatif/luxe ») → **refonte EN COURS sous le nouveau régime** avec cette direction.
