...
**⚠️ Les 4 sections du run from-scratch = REJETÉES par l'user 2026-07-03** (« dégueulasses les 3 [repères/approche/contact]… ça fait vraiment cheap » ; #viz : « PowerPoint fait sous Paint ; trop d'infos, dépasse l'écran ; l'asset doit être bien plus qualitatif/luxe »). Domaines = restauré version originale + VERROUILLÉ (commit d11c49d). **Refontes sous le NOUVEAU régime, EN FILE (une à la fois — même fichier) : #viz EN COURS (designer-viz-luxe) → puis #reperes → #approche → #contact.** Direction user globale pour les 3 : exécution nettement plus qualitative/luxe, moins « cheap » ; chaque refonte passe les filtres puis gate user en page live.
